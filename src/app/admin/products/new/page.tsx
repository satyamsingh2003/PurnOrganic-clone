...
export default function AddProductPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Upload image to Vercel Blob if selected
      ...
      const response = await fetch('/api/products', {
        ...
        body: JSON.stringify({
          ...formData,
          slug: generatedSlug,
          image: imageUrl || 'https://uvb9swoktkk6ztcv.public.blob.vercel-storage.com/category_daal.png', // fallback
          link: `/products/${generatedSlug}`
        }),
      });

      if (response.ok) {
        router.push('/admin/products'); // Redirect back to products list
      } else {
        ...
        <div className={styles.mainCol}>
          ...
          <section className={styles.panel}>
            ...
            <div className={styles.row3}>
              <div className={styles.inputGroup}>
                <label>Unit</label>
                <input type="text" name="unit" value={formData.unit} onChange={handleChange} placeholder="e.g. 500g, 1 kg, 250ml" />
              </div>
              <div className={styles.inputGroup}>
