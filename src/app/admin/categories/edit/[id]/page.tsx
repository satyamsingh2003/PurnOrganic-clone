...
export default function EditCategoryPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    fetch('/api/categories')
      ...
      .then(data => {
        if (data.categories) {
          ...
          setParentCategories(data.categories.filter((c: any) => String(c.id) !== unwrappedParams.id));
        }
      })
      .catch(err => console.error('Failed to fetch categories:', err));

    // Fetch this category
    fetch(`/api/categories/${unwrappedParams.id}`)
      .then(res => {
      ...
      .then(cat => {
        setFormData({
          ...
          sortOrder: cat.sort_order ? String(cat.sort_order) : '0',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(`/api/categories/${unwrappedParams.id}`, {
        method: 'PUT',
  ...
  if (fetching) {
    return <div style={{ padding: '2rem' }}>Loading category details...</div>;
  }
